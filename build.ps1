...
if ($env:ADDON) {
    ...
    if ($env:ADDON -eq "asan") {
        Add-Content -Path $TRIPLET_DEST -Value "if(NOT `"`${PORT}`" MATCHES `"botan|openssl|libiconv`")"
    }
...
Invoke-ExternalCommand {
    vcpkg\vcpkg --overlay-ports=ports install ${VCPKG_DEBUG_OPTION} docwire[tests]:${VCPKG_TRIPLET}
}
